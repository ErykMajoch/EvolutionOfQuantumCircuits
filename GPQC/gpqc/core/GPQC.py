...
class GPQC:
    ...
    def evolve(self, plot_results: bool = False) -> None:
        ...
        for generation in range(self.max_generations):
            ...
            crossover_rate = self.crossover_rate * self.adaptive_rate
            mutation_rate = self.mutation_rate * self.adaptive_rate
            max_mutations = max(
                1, int(self.num_qubits * self.max_depth * mutation_rate * 0.1)
            )
            ...
            while len(new_population) < self.population_size:
                ...
                mutation_type = random.choices(
                    ["replace", "parameter", "delete"], weights=[0.5, 0.3, 0.2]
                )[0]
                child.mutate(
                    mutation_rate,
                    mutation_type,
    ...
    def _calculate_adaptive_rate(self) -> None:
        ...
        """
        # Linear adaptive rate
        self.adaptive_rate = -(self.current_generation + 1 / self.max_generations) + 1

        # Exponential decay adaptive rate
        # self.adaptive_rate = np.exp(
        #     (-self.current_generation + 1) / (self.max_generations / 3)
        # )
